...
const QUERY_STOPWORDS = new Set([
  "azienda","aziende","impresa","imprese","srl","spa","snc","sas","italia","italy",
  "lavorazione","lavorazioni","materie","plastiche","plastica","cnc","taglio","laser",
  "incisione","incisioni","targhe","serigrafia","grafica","insegne","supporti","rigidi",
  "plexiglass","plexiglas","pmma","pvc","petg","policarbonato","acrilico","acrilici",
  "fresatura","tornitura","produzione","fabbricazione","servizi","fornitura","fornitore",
  "di","del","della","dei","delle","a","al","alla","in","con","per","e"
]);

function normalizedTokenText(s = "") {
  return String(s || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function queryDistinctiveTokens(query = "") {
  return normalizedTokenText(query)
    .split(" ")
    .filter((t) => t.length >= 4 && !QUERY_STOPWORDS.has(t));
}

function queryHostBrand(query = "", url = "") {
  const host = domainText(url).split(".")[0] || "";
  const hostFlat = normalizedTokenText(host).replace(/ /g, "");
  if (!hostFlat) return "";

  const originalTokens = cleanSearchTitle(query).match(/[A-Za-zÀ-ÿ0-9'’.-]+/g) || [];
  const hits = originalTokens.filter((tok) => {
    const n = normalizedTokenText(tok).replace(/ /g, "");
    return n.length >= 4 && hostFlat.includes(n);
  });

  // Un dominio ufficiale spesso concatena brand e attività, es. incisionimorlacchi.it.
  // Richiediamo almeno due token coerenti con la query per evitare di trasformare
  // una directory o una località nel nome dell'azienda.
  if (hits.length >= 2) return hits.slice(0, 4).join(" ");

  const distinctive = queryDistinctiveTokens(query);
  const strongHit = distinctive.find((t) => t.length >= 7 && hostFlat.includes(t));
  if (strongHit) {
    const original = originalTokens.find((tok) => normalizedTokenText(tok) === strongHit);
    return original || strongHit;
  }

  return "";
}

function descriptiveTitlePart(part = "") {
  const p = cleanSearchTitle(part).trim();
  if (!p) return true;
  if (isGenericCompanyTitle(p)) return true;
  if (/^(?:home|homepage|benvenuti|welcome|chi siamo|contatti|orari(?: di)? apertura|dove siamo|come arrivare|mappa|indicazioni|sede|sedi)$/i.test(p)) return true;
  if (/^(?:orari(?: di)? apertura|contatti?|dove siamo|come arrivare|mappa(?: e contatti)?|indicazioni|sede|sedi)\b/i.test(p)) return true;
  if (/^(?:lavorazione|lavorazioni|produzione|fabbricazione|allestimenti|servizi|specialisti|materie plastiche|articoli domestici)\b/i.test(p) && !/\b(?:s\.?r\.?l\.?|s\.?p\.?a\.?|s\.?n\.?c\.?|sas|snc|spa|srl)\b/i.test(p)) return true;
  return false;
}

function companyNameFromPageLabel(rawTitle = "", query = "") {
  const title = cleanSearchTitle(rawTitle).trim();
  if (!title) return "";

  // Esempi: 'Orari di apertura "Incisioni Morlacchi"', 'Contatti - Azienda X'.
  const prefixed = title.match(/^(?:orari(?: di)? apertura|contatti?|dove siamo|come arrivare|mappa(?: e contatti)?|indicazioni|sede|sedi)\s*(?:[-:|–—]\s*)?["“'«]?(.{3,100}?)["”'»]?\s*$/i);
  if (prefixed) {
    const candidate = cleanSearchTitle(prefixed[1]).trim();
    if (candidate && !descriptiveTitlePart(candidate) && !isGenericCompanyTitle(candidate)) return candidate;
  }

  // Directory: 'Incisioni Morlacchi, Orio al Serio' -> conserva il brand se
  // la parte prima della virgola coincide con i token distintivi della query.
  const commaParts = title.split(/\s*,\s*/).filter(Boolean);
  if (commaParts.length > 1) {
    const first = commaParts[0].trim();
    const qTokens = queryDistinctiveTokens(query);
    const firstNorm = normalizedTokenText(first);
    const overlap = qTokens.filter((t) => firstNorm.includes(t)).length;
    if (overlap >= 1 && first.split(/\s+/).length <= 8 && !descriptiveTitlePart(first)) return first;
  }

  return "";
}

function companyNameFromResult(rawTitle = "", url = "", query = "") {
  const title = cleanSearchTitle(rawTitle).trim();
  const hostBrand = queryHostBrand(query, url);
  const pageLabelBrand = companyNameFromPageLabel(title, query);

  // Se il dominio ufficiale contiene chiaramente il brand cercato, quello è
  // l'identificativo più affidabile. Evita di creare più aziende dalla stessa
  // società per pagine come "Orari di apertura", "Contatti", indirizzi, ecc.
  if (hostBrand && !BUSINESS_DIRECTORY_HOST.test(domainText(url))) return hostBrand;
  if (pageLabelBrand) return pageLabelBrand;

  if (!title) return hostBrand;

  const parts = title.split(/\s+[|–—-]\s+/).map((x) => x.trim()).filter(Boolean);
  if (parts.length > 1) {
    const legal = parts.find((p) => /\b(?:s\.?r\.?l\.?|srls|s\.?p\.?a\.?|s\.?n\.?c\.?|snc|s\.?a\.?s\.?|sas)\b/i.test(p));
    if (legal && !descriptiveTitlePart(legal)) return legal;

    const queryTokens = queryDistinctiveTokens(query);
    const brandish = parts
      .filter((p) => !descriptiveTitlePart(p))
      .map((p) => ({
        p,
        overlap: queryTokens.filter((t) => normalizedTokenText(p).includes(t)).length,
        words: normalizedTokenText(p).split(" ").filter(Boolean).length,
      }))
      .sort((a, b) => b.overlap - a.overlap || a.words - b.words);

    if (brandish.length && (brandish[0].overlap > 0 || brandish[0].words <= 5)) return brandish[0].p;
  }

  if (!descriptiveTitlePart(title)) {
    const distinct = queryDistinctiveTokens(query);
    if (!distinct.length || distinct.some((t) => normalizedTokenText(title).includes(t)) || COMPANY_SIGNAL.test(title)) {
      return title;
    }
  }
...
function candidateScore(x: any, query = "") {
  ...
  const hostBrand = queryHostBrand(query, x.url);
  const distinctiveQueryTokens = queryDistinctiveTokens(query);
  const queryOverlap = distinctiveQueryTokens.filter((t) => normalizedTokenText(blob).includes(t)).length;
  const company = COMPANY_SIGNAL.test(title) || COMPANY_SIGNAL.test(host) || !!hostBrand || queryOverlap >= 1;
  ...
  if (format) score += 3;
  if (company) score += 5;
  if (queryOverlap >= 1) score += 6;
  if (hostBrand) score += 10; // forte segnale di sito ufficiale coerente con la query
...
function companyIdentityFromTitle(raw = "") {
  ...
  t = t
    ...
    .replace(/^(?:Orari(?: di)? apertura|Contatti?|Dove siamo|Come arrivare|Mappa(?: e Contatti)?)\s*[-:|–—]?\s*["“'«]?/i, "")
    .replace(/["”'»]\s*$/i, "")
...
function isAddressOrLocationOnlyTitle(raw = "") {
  const title = cleanSearchTitle(String(raw || "")).trim();
  if (!title) return true;

  // Indirizzi/località restituiti da directory o motori non sono aziende.
  const hasLegalForm = /\b(?:s\.?\s*r\.?\s*l\.?|srls|s\.?\s*p\.?\s*a\.?|s\.?\s*n\.?\s*c\.?|snc|s\.?\s*a\.?\s*s\.?|sas)\b/i.test(title);
  const hasCompanySignal = COMPANY_SIGNAL.test(title) || ACTIVE_PROCESS_SIGNAL.test(title) || VISUAL_PLASTIC_BUYER_SIGNAL.test(title);
  if (hasLegalForm || hasCompanySignal) return false;

  // Esempi da scartare: "Orio al Serio", "Orio al Serio, Via Galileo Galilei, 16".
  if (/^(?:[A-ZÀ-Ý][A-Za-zÀ-ÿ'’.-]+(?:\s+(?:al|alla|alle|ai|a|di|del|della|dei|delle|in|sul|sulla|san|santa|santo))?\s*){1,6}(?:,\s*)?(?:(?:via|viale|piazza|corso|strada|largo|vicolo|localit[aà]|loc\.?)\b.*)?$/i.test(title)) {
    if (/(?:,\s*)?(?:via|viale|piazza|corso|strada|largo|vicolo|localit[aà]|loc\.?)\b/i.test(title)) return true;
  }

  if (/^(?:via|viale|piazza|corso|strada|largo|vicolo|localit[aà]|loc\.?)\s+.{2,120}(?:,|\s)\s*\d+[A-Za-z]?$/i.test(title)) return true;
  if (/^\d{5}\s+[A-Za-zÀ-ÿ'’ .-]{2,80}(?:\s*\([A-Z]{2}\))?$/i.test(title)) return true;

  // Nome località + indirizzo/CAP/civico senza alcun segnale aziendale.
  if (/(?:\bvia\b|\bviale\b|\bpiazza\b|\bcorso\b|\bstrada\b|\b\d{5}\b|,\s*\d+[A-Za-z]?\s*$)/i.test(title) && !hasCompanySignal) return true;
...
function rankResults(items: any[], query = "") {
  ...
  for (const x of items) {
    ...
    const companyName = companyNameFromResult(x.title, x.url, query);
    if (!companyName || isGenericCompanyTitle(companyName) || isAddressOrLocationOnlyTitle(companyName)) continue;

    // Le directory possono restituire località, indirizzi o intestazioni di pagina
    // al posto della ragione sociale. Se il "nome" non ha alcun segnale aziendale
    // o di attività target, non lo trasformiamo in prospect.
    if (BUSINESS_DIRECTORY_HOST.test(domainText(x.url))) {
      const nameBlob = `${companyName} ${String(x.title || "")}`;
      const plausibleDirectoryCompany =
        COMPANY_SIGNAL.test(nameBlob) ||
        ACTIVE_PROCESS_SIGNAL.test(nameBlob) ||
        VISUAL_PLASTIC_BUYER_SIGNAL.test(nameBlob) ||
        /\b(?:s\.?r\.?l\.?|srls|s\.?p\.?a\.?|s\.?n\.?c\.?|snc|s\.?a\.?s\.?|sas)\b/i.test(nameBlob);
      if (!plausibleDirectoryCompany) continue;
    }
    ...
    candidates.push({
      ...
      companyIdentity: companyIdentityFromTitle(companyName),
    });
  }
...
function namedBrandSearchPhrase(query = "") {
  const tokens = cleanSearchTitle(query).match(/[A-Za-zÀ-ÿ0-9'’.-]+/g) || [];
  if (tokens.length < 2) return "";

  const activityRx = /^(?:incisioni?|incisoria|grafica|serigrafia|insegne?|targhe?|targhette|pubblicit[aà]|visual|plexiglas|plexiglass|plastica|plastiche)$/i;
  for (let i = 0; i < tokens.length - 1; i++) {
    if (!activityRx.test(tokens[i])) continue;
    for (let j = i + 1; j < Math.min(tokens.length, i + 4); j++) {
      const n = normalizedTokenText(tokens[j]);
      if (n.length >= 4 && !QUERY_STOPWORDS.has(n)) {
        return `${tokens[i]} ${tokens[j]}`.trim();
      }
    }
  }
  return "";
...
async function directBrandProbe(query = "") {
  const phrase = namedBrandSearchPhrase(query);
  if (!phrase) return [];

  const slug = normalizedTokenText(phrase).replace(/\s+/g, "");
  if (slug.length < 7 || slug.length > 50) return [];

  const brandTokens = queryDistinctiveTokens(phrase);
  const urls = [
    `https://${slug}.it/`,
    `https://www.${slug}.it/`,
    `https://${slug}.com/`,
  ...
  for (const url of urls) {
    try {
      ...
      const plain = decodeEntities(text).slice(0, 180000);
      const host = domainText(url);
      const pageNorm = normalizedTokenText(plain);
      const overlap = brandTokens.filter((t) => pageNorm.includes(t)).length;
      if (!overlap) continue;

      const titleMatch = text.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
      const title = cleanSearchTitle(titleMatch?.[1] || phrase);
      return [normalizeResult({
        title: title || phrase,
        url,
        description: plain.slice(0, 700),
      }, "Direct brand probe")];
    } catch {}
  }
...
function queryVariants(q: string) {
  ...
  // Prima proviamo la query esatta senza il suffisso generico. Questo è fondamentale
  // per ricerche di aziende note (es. "Incisioni Morlacchi Orio al Serio"), perché
  // Bing/DDG possono degradare molto i risultati quando aggiungiamo parole come "srl azienda".
  variants.add(clean);

  // Per ricerche di un'azienda nominata (es. "Incisioni Morlacchi Orio al Serio")
  // aggiungiamo anche la coppia attività+brand. I motori possono cambiare molto
  // i risultati della query completa; questa variante rende la ricerca più stabile.
  const namedPhrase = namedBrandSearchPhrase(clean);
  if (namedPhrase) {
    variants.add(`"${namedPhrase}"`);
    variants.add(namedPhrase);
  }
...
async function search(q: string) {
  ...
  // Se i motori non restituiscono il sito ufficiale per una query aziendale
  // nominativa, proviamo in modo controllato il dominio attività+brand.
  // Non è hard-coded su Morlacchi: vale solo quando la query contiene un
  // pattern attività + nome distintivo e la pagina trovata contiene il brand.
  if (results.length === 0 && namedBrandSearchPhrase(q)) {
    const direct = await directBrandProbe(q);
    if (direct.length) {
      raw.push(...direct);
...
async function selftest() {
  ...
  const morlacchiFound = samples.some((x) => /morlacchi/i.test(String(x?.title || "") + " " + String(x?.url || "")));
  const ok = searchResults >= 3 && fetchSuccess >= 2 && targetPages >= 2 && buyerPages >= 2 && morlacchiFound;

  return {
    ok,
    morlacchiFound,
    engine: "supabase-edge-v8-clean-company-results",
    rawResults,
    searchResults,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "health") {
      return json({ ok: true, engine: "supabase-edge-v8-clean-company-results" });
    }
